...
If you find a bug or unexpected behavior, please submit a report using the
[Bug Report template](https://github.com/ptt/ptt-term/issues/new?template=bug_report.yml).

When filing a bug, please provide the **Build Info** (commit hash and build date)
instead of just the release version number, as it is much more useful for
debugging:
- **How to find it**: Open **Settings** (right-click anywhere or tap toolbar
  menu ➜ **Settings**) ➜ switch to the **About** tab ➜ copy the
  **Build: `<hash>` `<date>`** line.
