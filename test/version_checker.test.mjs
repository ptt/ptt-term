...
test('canCheckVersion supports 6h disconnect interval and 12h connect interval', () => {
  const storage = createMockStorage();
  const t0 = 1_700_000_000_000;

  storage.setItem(STORAGE_KEY_LAST_CHECK, String(t0));

  // At 5 hours: neither disconnect (6h) nor connect (12h) should check
  ...
  // At 6 hours: disconnect (6h) CAN check, but connect (12h) still CANNOT
  const t6h = t0 + DISCONNECT_CHECK_INTERVAL_MS;
  assert.equal(canCheckVersion({ now: t6h, intervalMs: DISCONNECT_CHECK_INTERVAL_MS, storage }), true);
  assert.equal(canCheckVersion({ now: t6h, intervalMs: CONNECT_CHECK_INTERVAL_MS, storage }), false);

  // At 12 hours: both disconnect (6h) and connect (12h) CAN check
  const t12h = t0 + CONNECT_CHECK_INTERVAL_MS;
...
test('checkVersionAndReloadIfNeeded respects intervals and prevents reload loops', async () => {
  ...
  // 2. Disconnect at 5 hours -> skips fetch
  const t5h = t0 + 5 * 3600_000;
  ...
  // 3. Disconnect at 6 hours -> fetches once, updates lastCheck to t6h, no reload if commit matches
  const t6h = t0 + DISCONNECT_CHECK_INTERVAL_MS;
  ...
  // 4. Reconnect at t6h + 8h (8h since lastCheck < 12h CONNECT_CHECK_INTERVAL_MS) -> skips fetch
  ...
  // 5. Reconnect at t6h + 12h -> fetches and reloads when server has new commit
  remoteCommit = 'commit_v2';
  const t18h = t6h + CONNECT_CHECK_INTERVAL_MS;
  ...
  assert.equal(storage.getItem(STORAGE_KEY_LAST_RELOAD), String(t18h));
...
test('App checks version on disconnect (6h) and connect (12h) without visibilitychange check', () => {
